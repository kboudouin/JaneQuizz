...
export default function Question({ question, nextQuestion, selectedType, currentQuestionIndex, questionsLenght, endGame, userAnswers, setUserAnswers, score, setScore}) {
...
//10 seconds countdown, when countdown runs out = endGame()
   ...
   const handleAnswer = (selectedOption) => {
    ...
    return (
      ...
      <h1 className="text-xl font-bold"> {currentQuestionIndex+1}/{questionsLenght}</h1>
      <button className="btn btn-2xl text-3xl btn-error mr-12">{timer}</button>
      </div>
                <div className=" flex card-body items-center text-center text-3xl font-bold ">
                    ...
                    <div className="card-actions flex flex-wrap">
                        {question.options.map((option, idx) => (
                            <button key={idx} className="btn btn-secondary" onClick={() => handleAnswer(option)}>
                                {option}
                            </button>
                        ))}
                    </div>
