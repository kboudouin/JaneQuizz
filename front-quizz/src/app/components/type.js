  ...
  return (
    <div className='p-6 h-screen'>
      {quizTypes.map((type, index) => (
        <div key={index} className="stats m-3 bg-primary text-primary-content transform transition duration-500 hover:scale-105">
          <div className="stat">
            <div className="stat-value">{type}</div>
            <div className="stat-actions">
              {/* Call the StartQuizz function from Quizz Page when clicked */}
              <button onClick={() => startQuiz(type)} className="btn btn-sm btn-success">Play</button>
            </div>
          </div>
        </div>
      ))}
